...
class UebiMiau_core {
	...
	*/
	...
	function process_message($header,$body) {
		...
		case "text":
			...
			break;
		case "multipart":
			// CDM added alternative
			if(ereg($subtype,"alternative,signed,mixed,related,report,appledouble"))
				$subtype = "complex";
	...
	*/
	...
	function build_attach($header,$body,$boundary,$part) {
		...
		// JFillmore, 3/20/08. Recent versions of PHP return false when the string length is less than the absolute value of the passed length, so only truncate if needed
	...
	*/
	function build_mime_date( $mydate, $timezone = "+0000" ) {
		...
		# Fixed by JFillmore on 6/5/08 to properly handle time zones.
