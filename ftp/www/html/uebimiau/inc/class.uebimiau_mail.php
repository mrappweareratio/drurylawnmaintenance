	...
	function mail_list_msgs($boxname = "INBOX") {
		...
		*/
		...
		for($i=0;$i<count($messages);$i++) {
			...
			// CDM check for apple bad attachment syntax
			if ($messages[$i]["attach"] == 0 && eregi("multipart",$mail_info["content-type"]) && eregi("Apple", $mail_info["content-type"])) $messages[$i]["attach"] = 1;
